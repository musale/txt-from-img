...
rm -rf layers
mkdir -p layers/python/bin
mkdir -p layers/{lib,bin,data}
docker build $DOCKER_ARG --build-arg TESSERACT_LANG="$TESSERACT_LANG" --build-arg TESSERACT_MODE="$TESSERACT_MODE" -t musale/txt-from-img -f Dockerfile .
CONTAINER=$(docker run -d musale/txt-from-img false)
